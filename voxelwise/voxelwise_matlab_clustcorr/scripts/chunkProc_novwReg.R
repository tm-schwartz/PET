...
library(tools)
library(FRACTION)
#Uncomment nlme library if not running mixed effects
library(nlme)


#Model function
runModel <- function(y,covDF) {
  ...
  #try(m <- lm(y ~  age+fsrp.minus.age.points+education+sex.factor+raceethnicity.factor+apoe4pos.factor+pwv+diagnosis.factor, data=covDF,na.action=na.omit),silent=TRUE)

  #try(m <- lm(y ~ age, data=covDF,na.action=na.omit),silent=TRUE)

  #Example lme with voxelwise-regressor
  cntrl<-lmeControl(maxIter = 10000, msMaxIter = 10000, opt = "optim")
  #simpler model, only random intercept
  try(m <- lme(y ~ pre_post + DeltaPost + Sex + AgeAtDiagnosis+ Ethnicity, data = covDF, na.action=na.omit, random=~1 | Patient, control = cntrl),silent=TRUE)
...
parseCL <- function(args) {
  ...
  parsedArgs <- c(inImg,chunkFolder,chunkNumber,nChunks,maskImg,covFile,outPath)
  return(parsedArgs)
} 
...

{
  ...
  chunkNumber <- as.numeric(parsedArgs[3])
  nChunks <- as.numeric(parsedArgs[4])

  #Gather mask dimension information
  maskImg <- parsedArgs[5]
  ...
  
  {
    ...
    result <- parLapply(cl,seq_len(nrow(voxVectors)), function(x) {
      return(runModel(voxVectors[x,],covDF=covFile))
    })
    stopCluster(cl)
    end.time <- Sys.time()
