...
runModel <- function(y,vw_reg,covDF) {
  ...
  try(m <- lm(y ~ vw_reg + cdrBin + age+fsrp.minus.age.points+education+sex.factor+raceethnicity.factor+apoe4pos.factor+pwv, data=covDF,na.action=na.omit),silent=TRUE)
  #try(m <- lm(y ~ vw_reg + cdrBin, data=covDF,na.action=na.omit),silent=TRUE)
...

{
  ...
  
  {
    ...
    result <- parLapply(cl,seq_along(nrow(voxVectors)), function(x) {
      runModel(voxVectors[x,],vwRegVectors[x,],covDF=covFile)
    })
    stopCluster(cl)
    end.time <- Sys.time()
