...
#script will set up contrast and design matrices for fsl randomise without using fsl gui
#can use gui on small mock example to make sure you're doing things correctly

#if using rm-anova, will need to add on outputting exchanability blocks
#not sure if randomise supports rm-ancova but it probably does
#can also look into using FSL PALM, which is similar tool
...
df<-readRDS("/scratch/Hudson/resource/mapDatamerge/MAP_fm_f20230306_m20230306.rds")
#read in qei
...
#remove 034 and 048 because they have misaligned M0 and ASL data
df <- dplyr::filter(df,map.id!="034")
df <- dplyr::filter(df,map.id!="048")

#remove 247 due to signal dropout
df <- dplyr::filter(df,map.id!="247")
...
cols <- c("map.id", "epoch","asl.session.id","apoe4pos.factor","apoe4count","apoe2count","cdr.factor","diagnosis.factor","age",
          ...
          "ma.grey.matter","ma.total.intracranial.vol",
          "csf.fujirebio.ptau.2022","asl.rest.grey.matter.hct","asl.rest.grey.matter.hct.scov")
...
df_base <- dplyr::filter(df_base,asl.rest.grey.matter.hct.scov<0.5)

#filter for data we want

cols <- c("map.id", "epoch","asl.session.id","apoe4pos.factor","diagnosis.factor","age",
          "raceethnicity.factor","fsrp.minus.age.points","sex.factor","raceethnicity.factor",
          "education","diagnosis.factor","csf.fujirebio.ptau.2022","asl.rest.grey.matter.hct.scov")
...
print("after removing duplicate mapids")
print(nrow(df_base))
...
predictor <- "csf.fujirebio.ptau.2022"
...
variables_to_meancenter <- c("age","fsrp.minus.age.points",predictor, "factorized")
...
#will need to be edited for epochs where there is diagnosis of dementia
