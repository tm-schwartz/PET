...
def get_suvbw_scale_factor(sidecar: str | Path) -> float:
    ...
    series_time = datetime.strptime(
        sidecar_json.get("SeriesTime").split(".")[0], "%H%M%S"
    )
...
def compute_suv_volumes(
    ...
    reference_region_mask = math_img(
        "np.where(mask == 0.0, np.nan, 1.0)", mask=reference_region
    )
    out_path_suv_image = input_volume_path.parents[0].joinpath(
        input_volume_path.name.replace(".nii.gz", "-SUV.nii.gz")
    )
    ...
    if pons_vermis_reference_region:
        suv_reference_region_array = (
            math_img(
                "suvimg * rr_mask", suvimg=suv_image, rr_mask=reference_region_mask
            )
            .get_fdata()
            .flatten()
        )
        suv_reference_region = suv_reference_region_array.compress(
            ~np.isnan(suv_reference_region_array)
        )
    ...
    math_img(f"suvimg / {suv_reference_region_mean}", suvimg=suv_image).to_filename(
        out_path_suvr_image
    )
    return StandardizedData(
        out_path_suv_image, out_path_suvr_image, suv_reference_region_mean, template_dir
    )
...
def get_roi_suvr_means(data_class: StandardizedData) -> str:
    ...
    out_file = data_class.suv_path.parent.joinpath(
        data_class.suv_path.name.split(".")[0].replace("SUV", "SUVR") + ".csv"
    )
    ...
    for i, roi in enumerate(roi_masks):
        columns.append(
            re.sub(r".nii.gz|-mask|\,|\\", "", roi.name)
            .replace("harvaroxford", "harvox")
            .replace("cortical", "cort")
        )
        ...
        mean_vals[i] = (
            np.nanmean(
                math_img(
                    "suv_image * roi_mask", suv_image=suv_image, roi_mask=roi_mask
                ).get_fdata()
            )
