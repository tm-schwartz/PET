...
input_image_path = next(impresources.files(pet_py).parents[1].glob("resources/*input_img.nii.gz"))
input_json_path = next(impresources.files(pet_py).parents[1].glob("resources/*input_img.json"))
resampled_image_path = next(impresources.files(pet_py).parents[1].glob("resources/*pixdim*.nii.gz"))
synthstrip_image_path = next(impresources.files(pet_py).parents[1].glob("resources/*stripped-synthstrip*.nii.gz"))
registered_image_path = next(impresources.files(pet_py).parents[1].glob("resources/*elastix*.nii.gz"))
pickled_data_obj = impresources.files(pet_py).parents[1].joinpath("resources","standardized_data.pickle")
...
fixed_volume_path = next(
    impresources.files(pet_py).parents[1].glob("resources/MNI152_PET_1mm_coreg_stripped_smoothed.nii.gz")
)
param_file = str(next(impresources.files(pet_py).parents[1].glob("resources/param27.txt")))


def test_function():
    sidecar = json.loads(input_json_path.read_text())
    sd = "_".join([*sidecar["SeriesDescription"].split(), *sidecar["ImageType"], sidecar["StationName"]])
    assert (
...
def test_compute_suv_volumes():
    ...
    out_data = pet_py.analysis.compute_suv_volumes(registered_image_path, suv_scale_factor, templates_path, next(templates_path.glob("*ponsvermis_1mm_bin*.nii.gz")))
